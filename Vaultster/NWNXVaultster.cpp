...
char* CNWNXVaultster::OnRequest(char* gameObject, char* request, char* parameters)
{
	...
	if (cmd == Status) {
		...
		{
			Log("o Error status cleared for client %d.\n", job);
		...
		Log("o Looking for a client for job: %s\n", parameters);
		for (i = 0; i < maxClients; i++) {
			if (clients[i].getStatus() == STATUS_OK)
				break;
			else if(clients[i].getStatus() == STATUS_BUSY)
				Log("o Client %d busy.\n", i);
			else if(clients[i].getStatus() == STATUS_ERROR)
				Log("o Client %d had an error.\n", i);
		}
