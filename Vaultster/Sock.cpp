...
BOOL CSock::SendFile (char* File, BOOL bCompress)
{
   ...
   vaultster.Log ("o Sending data part of the file (%d bytes).\n", actualDataSize);
	...
	free (sendData);
	Receive (code);
	vaultster.Log ("o Received status code %i, exiting SendFile.\n", code);
	return (bool)code;
}
...
BOOL CSock::ReceiveFile (char* File, BOOL bCompressed)
{
	...
	vaultster.Log ("o Received file, waiting to write: %s\n", File);
   ...
   if (f == NULL) {
	  ...
	  vaultster.Log ("o Could not open file. Trying to create dir: %s\n", File);

      if (!CreateDirectory (File, NULL)) {
		 vaultster.Log("o Failed to create directory %s. Sending failure code.\n", File);
         Send (0);
		   return FALSE;
      }
      ...
      f = fopen (File, "wb");
      if (f == NULL) {
		 vaultster.Log("o Failed to open file %s. Sending failure code.\n", File);
         Send (0);
         return FALSE;
      }
   }

   vaultster.Log("o File opened for writing: %s\n", File);
	...
	// signal ready
    vaultster.Log("o Saved file, returning success: %s\n", File);
	Send (1);
	return TRUE;
}
